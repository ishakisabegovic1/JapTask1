  ...
  public partial class spAdminReport : Migration
  {
    protected override void Up(MigrationBuilder migrationBuilder)
    {
      string procedure = @"Create procedure [dbo].[spAdminReportTab]
                                as
                                Begin
                                	SELECT s.Name as SelName,  p.Name as PrName, IsNull(numberOfStudents,0) as numberOfStudents, IsNull(numberOfSuccess,0) as numberOfSuccess, IsNull((numberOfSuccess*100/numberOfStudents),0) as SuccessRate
								  FROM [HAJDE.db].[dbo].Selections  s 

								  INNER JOIN [HAJDE.db].[dbo].Programs  p ON p.Id = s.JapId 
